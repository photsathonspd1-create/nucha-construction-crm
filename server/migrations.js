// ===== Database Migrations =====
// NOTE: All db methods are async (Supabase proxy) — must use await
const db = require('./db');
...
async function runMigrations() {
  ...
  let applied = [];
  try {
    const result = await db.prepare('SELECT name FROM migrations').all();
    applied = Array.isArray(result) ? result.map(r => r.name) : [];
  } catch (err) {
    console.log('  ℹ️  migrations table not found — run the SQL below in Supabase SQL Editor first:');
    console.log(`
...
CREATE TABLE IF NOT EXISTS migrations (
  ...
  applied_at TIMESTAMPTZ DEFAULT NOW()
    ...
    `);
    console.log('  ⏭️  Skipping migrations for now...');
    return;
  }
  ...
  const migrations = [
    {
      ...
      type: 'ddl',
      sql: `ALTER TABLE users ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'sales'`
    },
    {
    ...
    },
    {
      ...
      type: 'data',
      sql: async () => {
        const existing = await db.prepare("SELECT content FROM site_content WHERE section_key = 'notification_settings'").get();
        if (!existing) {
          const settings = {
          ...
          await db.prepare("INSERT INTO site_content (section_key, content, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)")
      ...
      sql: async () => {
        ...
        if (existing) {
          ...
          if (!config.line_channel_access_token) {
            ...
            await db.prepare("UPDATE site_content SET content = ?, updated_at = CURRENT_TIMESTAMP WHERE section_key = 'notification_settings'")
    ...
    },
    {
      ...
      type: 'ddl',
      sql: `ALTER TABLE chat_messages ADD COLUMN IF NOT EXISTS admin_name TEXT`
    },
    {
      name: '010_add_services_image_url',
      type: 'ddl',
      sql: `ALTER TABLE services ADD COLUMN IF NOT EXISTS image_url TEXT DEFAULT ''`
    },
    {
    ...
    },
    {
      ...
      type: 'data',
      sql: async () => {
        const countResult = await db.prepare('SELECT COUNT(*) as c FROM service_gallery').get();
        if (countResult && countResult.c > 0) return;
        ...
        for (let i = 0; i < galleryItems.length; i++) {
          ...
          await db.prepare(`INSERT INTO service_gallery (service_category, title, description, image_url, image_type, sort_order) VALUES (?, ?, ?, ?, ?, ?)`)
            .run(item.cat, item.title, item.desc, item.url, item.type, i);
        }
      }
    },
    {
  ...
  for (const migration of migrations) {
    ...
    try {
      if (migration.type === 'ddl') {
        // DDL migrations need to run via Supabase SQL Editor
        // Log the SQL so developer can run it manually
        console.log(`  ⚠️  ${migration.name} — DDL migration, run this SQL in Supabase SQL Editor:`);
        console.log(`     ${migration.sql.trim().substring(0, 100)}...`);
        // Still record as applied so it doesn't keep logging
        // Remove this line if you want to track which DDLs haven't been run yet
        // await db.prepare('INSERT INTO migrations (name) VALUES (?)').run(migration.name);
      } else {
        // Data migrations run through the proxy
        await migration.sql();
        await db.prepare('INSERT INTO migrations (name) VALUES (?)').run(migration.name);
